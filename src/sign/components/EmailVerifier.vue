...
import { getCaptcha, sendEmailCaptcha, ResponseBody } from "@/sign/api/sign";
...
const props = defineProps<{
  // 邮箱地址
  email: string;
  // 是否跳过邮箱验证 (修改邮箱时, 先对旧邮箱获取验证码, 这时会跳过旧邮箱的验证)
  disableValidateEmail?: boolean;
  // 是否不再弹示验证码已发送的提示
  disableToastSent?: boolean;
  // 占位符
  placeholder?: string;
  // 邮箱是否已存在 (对于通过邮箱找回密码的场景, 邮箱需要已注册在库)
  emailExistRequired?: boolean;
  /**
...
const validateCaptcha = async (id: string, payload: SlideCaptchaTrackData) => {
  return await sendEmailCaptcha(props.email, id, payload, props.emailExistRequired).catch(
...
const verifyEmail = () => {
  ...
  if (!props.disableValidateEmail && !isValidEmail(props.email)) {
