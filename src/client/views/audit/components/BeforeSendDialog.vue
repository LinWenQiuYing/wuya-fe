  ...
  >
    <div :class="$style._">
      <header :class="$style.title">
        <span>上传自定义的规则文件，支持docx格式文件</span>
        <el-link
          type="primary"
      ...
      <div :class="[$style.upload, checkFile && $style.cat_file]">
        ...
        >
          ...
          <el-link type="primary" :underline="false">上传规则文件</el-link>
        </el-upload>
        <template v-else>
      ...
      <p :class="$style.section">规则预览（若不上传则默认为内置模版）</p>
...
import { defaultAuditRule } from "@/client/const";
...
const defaultTemplate = ref<string>(defaultAuditRule);
...
watch(
  ...
  (container) => {
    ...
    const ops = parseMarkdownToOps(defaultTemplate.value, true);
...
const downTemplate = async () => {
  const content = defaultTemplate.value.replace("...", "").replace(/\n/g, "\n\n");
...
const send = () => {
  ...
  // 未对默认模板修改时 则传空值
  const markdown = htmlToMarkdown(html) === defaultTemplate.value ? "" : htmlToMarkdown(html);
...
const clearFile = () => (checkFile.value = null);
const close = () => {
  signal?.abort();
