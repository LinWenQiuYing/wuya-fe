<template>
  <el-form ref="formRef" :rules="formRules" :model="form" :class="$style._" @keydown.enter="onSubmit">
    ...
    <el-form-item prop="password">
      <el-input v-model="form.password" type="password" placeholder="密码(必填)" show-password />
    </el-form-item>
    <el-form-item prop="confirmPassword">
      <el-input v-model="form.confirmPassword" type="password" placeholder="确认密码(必填)" show-password />
    </el-form-item>
...
import { registerUs } from "@/client/api/user";
...
import PrivacyLink from "@/sign/components/PrivacyLink.vue";
import TermsLink from "@/sign/components/TermsLink.vue";
...
const formRules = reactive<FormRules<PasswordEditFormData>>({
  ...
  password: [{ validator: passwordQualityValidator, trigger: "change" }],
  confirmPassword: [
    { validator: getRequiredValidator("请确认密码"), trigger: "blur" },
    {
...
const onSubmit = async () => {
  ...
  const submitted = await registerUs(payload).catch((msg: string) => {
    ElMessage.error(msg);
  });
  submitting.value = false;
  if (!submitted) return;
  ElMessage.success("信息提交成功，审核通过后将会以邮件的方式发送");
  emit("next");
};
</script>
