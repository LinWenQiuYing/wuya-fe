    ...
    <div class="chat-items">
      ...
      >
        <div class="answer-text">
          ...
          <ChatSkeleton v-if="!item.answer.llm_output && !item.answer.reference && !item.answer.answerStop" />
          <div v-else>
...
import useSendServiceQuery, {
  ServiceParams,
} from "@/client/views/customerService/components/useSendServiceQuery";
import prepareService from "@/store/hooks/prepareService";

const { stopServiceChat, send } = useSendServiceQuery();
const chatData = computed(() => store.state.chat.serviceData); // 问答数据
...
const onClickSend = async (query: string) => {
  if (store.state.chat.isStartChat) stopServiceChat();
  const fixedQuestion = formatInputText(query);
  if (!fixedQuestion) return;
  prepareService(fixedQuestion);
