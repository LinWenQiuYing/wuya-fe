...
const PDFViewerApplication = {
  ...
  setInitialView(storedHash, {
    ...
    // todo 开启预览时 高亮指定文本 url中插入keyword
    var urlPath = decodeURIComponent(window.location.href);
    var startIndex = urlPath.indexOf('&keyword');
    const endIndex = urlPath.indexOf('#');
    if (startIndex !== -1) {
      var keyword = endIndex > 0 ? urlPath.substring(startIndex+9, endIndex) : urlPath.substring(startIndex+9);
